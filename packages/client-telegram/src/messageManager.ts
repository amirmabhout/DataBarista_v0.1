...
export class MessageManager {
     ...
     * @param ctx Context object containing the chat ID
    ...
    public async sendImage(
        ...
        try {
            // Use the existing sendMedia method with PHOTO type
            await this.sendMedia(ctx, imagePath, MediaType.PHOTO, caption);
            elizaLogger.info(`Successfully sent image to chat ${ctx.chat.id}`);
        } catch (error) {
            elizaLogger.error(`Error sending image to chat ${ctx.chat.id}:`, error);
        }
    }

    /**
     * Send an image directly to a chat ID
     * @param chatId The Telegram chat ID to send the image to
     * @param imagePath Path to the image file or URL
     * @param caption Optional caption for the image
     * @returns Promise that resolves when the image is sent
     */
    public async sendImageToChatId(
        chatId: string | number,
        imagePath: string,
        caption?: string
    ): Promise<void> {
        try {
            const isUrl = /^(http|https):\/\//.test(imagePath);
            
            if (isUrl) {
                // Handle HTTP URLs
                await this.bot.telegram.sendPhoto(chatId, imagePath, { caption });
            } else {
                // Handle local file paths
                ...
                const source = { source: fs.createReadStream(imagePath) };
                await this.bot.telegram.sendPhoto(chatId, source, { caption });
            }
            
            elizaLogger.info(`Successfully sent image to chat ID ${chatId}`);
        } catch (error) {
            elizaLogger.error(`Error sending image to chat ID ${chatId}:`, error);
        }
    }
}
